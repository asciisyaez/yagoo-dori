...
describe("native Leader resolution cache", () => {
  it("only groups within structural classes and keeps the complete graph key", () => {
    ...
    expect(result.counts.eligibleLeaderOutfits).toBe(124);
    expect(result.counts.structuralClasses).toBeGreaterThan(0);
    expect(result.counts.structuralClasses).toBeLessThanOrEqual(124);
    ...
    expect(new Set(result.groups.map((group) => group.eligibleCardIds.flat()).flat()).size).toBe(124);
  });
